...
export const WorkshopPage: React.FC = () => {
  ...
  const openJob = (job: WorkOrder) => {
    setSelectedJob(job);
    setWorkshopReport(job.technician_report ?? '');
  };
  ...
  const handleCompleteAndSign = async () => {
    if (!selectedJob) {
      return;
    }

    await completeForBilling(selectedJob.id, {
      report: workshopReport,
      signedName: profile?.full_name || t('common.admin_user'),
    });
        ...
        <div className="absolute inset-y-0 right-0 w-[450px] bg-white shadow-2xl border-l border-gray-200 z-20 flex flex-col transform transition-transform duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-white">
            {activeTab === 'info' && (
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">{t('wo.tech_report')}</label>
                  <textarea
                    value={workshopReport}
                    onChange={(event) => setWorkshopReport(event.target.value)}
                    placeholder={t('wo.describe_work')}
                    className="w-full min-h-[140px] p-3 bg-white border border-gray-200 rounded-lg text-sm"
                  />
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t border-gray-100 bg-white">
            <button
              ...
              className="w-full py-2.5 rounded-lg bg-docuraft-navy text-white text-sm font-semibold hover:bg-slate-800 transition-colors"
            >
              {t('wo.complete_sign')}
            </button>
          </div>
