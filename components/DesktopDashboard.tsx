...
import { JobStatus, JobPriority, WorkOrder } from '../types';
import { Search, Plus, ArrowRight, Activity, Map, MoreHorizontal, Calendar, Bell, Filter, ChevronRight, CheckCircle2, Clock, AlertCircle } from 'lucide-react';
...
export const DesktopDashboard: React.FC = () => {
  ...
  const doneCount = jobs.filter(j => j.status === JobStatus.DONE).length;
      ...
      <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-6 md:space-y-8">
        ...
        <section>
            <div className="flex justify-between items-end mb-4">
                ...
                <span className="text-xs md:text-sm text-slate-500 font-medium">Last 30 Days</span>
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                <div className="bg-white rounded-2xl shadow-card border border-docuraft-border overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-50">
                        {activeJobs.length === 0 ? (
                            <div className="p-8 text-center text-slate-400 italic">No scheduled jobs for today.</div>
                        ) : (
                            activeJobs.map((job) => (
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <h4 className="font-bold text-slate-900">{getCustomerById(job.customer_id)?.name || 'Unknown Customer'}</h4>
                                            ...
                                            <div className="flex items-center gap-4 text-xs font-medium text-slate-400">
                                                <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> 09:00 AM - 11:00 AM</span>
                                    ...
                                    <div className="flex items-center gap-4">
                                         <div className="text-right hidden sm:block">
                                            {job.assigned_to_user_id ? (
                                                ...
                                                <span className="text-xs text-orange-500 bg-orange-50 px-2 py-1 rounded font-bold">Unassigned</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="bg-white rounded-2xl shadow-card border border-docuraft-border overflow-hidden flex flex-col h-[280px]">
                    ...
                    <div className="flex-1 bg-slate-100 relative group cursor-pointer overflow-hidden">
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform">
                             <div className="bg-docuraft-navy text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg mb-1 whitespace-nowrap">Tech A</div>
                    ...
                    <div className="divide-y divide-gray-50">
                        {technicians.slice(0,3).map(tech => (
                            <div key={tech.id} className="p-3 flex items-center justify-between">
                                ...
                                <Button variant="outline" size="sm" className="h-7 text-xs px-3">Map</Button>
                            </div>
                        ))}
                    </div>
                </div>
...
const WorkflowCard = ({ label, count, color, icon, subtext }: any) => {
    ...
    return (
        <div className="bg-white p-5 rounded-2xl shadow-card border border-docuraft-border hover:shadow-md transition-all cursor-pointer group relative overflow-hidden">
