                  ...
                  <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar pb-20">
                    {columnJobs.map((job) => (
                      <JobCard
                        key={job.id}
                        job={job}
                        customerName={getCustomerById(job.customer_id)?.name || 'Unknown'}
                    ...
                    {columnJobs.length === 0 && (
                      <div className="text-center py-10 border-2 border-dashed border-gray-200 rounded-xl">
                        <p className="text-slate-400 text-sm font-medium">No jobs</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className={`bg-white border-l border-gray-200 transition-all duration-300 flex flex-col ${isTechSidebarOpen ? 'w-80' : 'w-0 overflow-hidden'}`}>
          <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
              {technicians.filter((tech) => tech.status === 'available').length} Available
            </span>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500 flex items-center gap-1">
                        <MapPin className="w-3 h-3" /> Area 1
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div className="bg-white p-4 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 hover:shadow-md hover:border-gray-300 transition-all cursor-grab active:cursor-grabbing group relative">
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide ${priorityClass}`}>{job.priority}</span>
